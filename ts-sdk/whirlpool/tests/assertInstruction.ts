import { getCreateAccountInstructionDataDecoder, getCreateAccountWithSeedInstructionDataDecoder, getTransferSolInstructionDataDecoder, SYSTEM_PROGRAM_ADDRESS } from "@solana-program/system";
import { ASSOCIATED_TOKEN_PROGRAM_ADDRESS, getCloseAccountInstructionDataDecoder, getCreateAssociatedTokenInstructionDataDecoder, getInitializeAccount3InstructionDataDecoder, getSyncNativeInstructionDataDecoder, TOKEN_PROGRAM_ADDRESS } from "@solana-program/token";
...
export function assertCreateAtaInstruction(instruction: IInstruction, checks: { ata: Address, idempotent?: boolean, owner: Address, mint: Address }) {
  ...
  try {
    const data = getCreateAssociatedTokenInstructionDataDecoder().decode(instruction.data ?? new Uint8Array());
...
export function assertCloseAccountInstruction(instruction: IInstruction, checks: { account: Address, owner: Address }) {
  ...
  try {
    const data = getCloseAccountInstructionDataDecoder().decode(instruction.data ?? new Uint8Array());
...
export function assertSolTransferInstruction(instruction: IInstruction, checks: { from: Address, to: Address, amount: bigint }) {
  assert.strictEqual(instruction.programAddress, SYSTEM_PROGRAM_ADDRESS);

  try {
    const data = getTransferSolInstructionDataDecoder().decode(instruction.data ?? new Uint8Array());
...
export function assertSyncNativeInstruction(instruction: IInstruction, checks: { account: Address }) {
  assert.strictEqual(instruction.programAddress, TOKEN_PROGRAM_ADDRESS);

  try {
    const data = getSyncNativeInstructionDataDecoder().decode(instruction.data ?? new Uint8Array());
...
export function assertCreateAccountInstruction(instruction: IInstruction, checks: { account: Address, payer: Address, owner: Address }) {
  assert.strictEqual(instruction.programAddress, SYSTEM_PROGRAM_ADDRESS);

  try {
    const data = getCreateAccountInstructionDataDecoder().decode(instruction.data ?? new Uint8Array());
...
export function assertCreateAccountWithSeedInstruction(instruction: IInstruction, checks: { account: Address, payer: Address, owner: Address, seed: string }) {
  assert.strictEqual(instruction.programAddress, SYSTEM_PROGRAM_ADDRESS);

  try {
    const data = getCreateAccountWithSeedInstructionDataDecoder().decode(instruction.data ?? new Uint8Array());
...
export function assertInitializeAccountInstruction(instruction: IInstruction, checks: { account: Address, mint: Address, owner: Address }) {
  assert.strictEqual(instruction.programAddress, TOKEN_PROGRAM_ADDRESS);

  try {
    const data = getInitializeAccount3InstructionDataDecoder().decode(instruction.data ?? new Uint8Array());
